...
//     waveform += sp.encoder.Increment();
//     waveform = DSY_CLAMP(waveform, 0, NUM_WAVEFORMS);
//     osc.SetWaveform(waveforms[waveform]);

//     if(sp.button2.RisingEdge())
//         octave++;
//     if(sp.button1.RisingEdge())
//         octave--;
...
{
    sp.ProcessDigitalControls();
    int wavelength = sp.encoder[1].Value();

    // Audio Loop
    for(size_t i = 0; i < size; i += 2)
    {
        // Process
        out[i] = out[i + 1] = count < 0 ? 1.0 : -1.0;
...
int main(void)
{
    ...
    sp.encoder[1].SetRange(1, 100);
    sp.encoder[1].ResetOnPressed();
    sp.encoder[1].SetValue(24);
